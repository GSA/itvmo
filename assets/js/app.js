...
if (document.getElementById("events-page") != null) {
  ...
  const d = urlParams.get("day"),
    m = urlParams.get("month") - 1,
    y = urlParams.get("year");
  ...
  displayEvents(
    document.getElementsByClassName("nav-list")[1],
    "past-events",
    pastEventList
  );
}
...
const filterMap = new Map([
  ["p-filter", "Policy"],
  ["acquisition-best-practices", "Acquisition Best Practices"],
  ["small-business", "Small Business"],
  ["market-intelligence", "Market Intelligence"],
  ["technology", "Technology"],
  ["contract-solutions", "Contract Solutions"],
  ["itvmo-general", "ITVMO General"],
]);
const filterColorMap = new Map([
  ["p-filter", "#f2938c"],
  ["acquisition-best-practices", "#59b9de"],
  ["small-business", "#abace5"],
  ["market-intelligence", "#3e4ded"],
  ["technology", "#ddaa01"],
  ["contract-solutions", "#5abf95"],
  ["itvmo-general", "#b04abd"],
]);
const bOfferingMap = new Map([
  ["acquisition-policy-it-category", "Acquisition Policy &  IT Category"],
  ["it-buyers-training-support", "IT Buyers Training & Support"],
  ["market-it-data-intelligence", "Market & IT Data Intelligence"],
  ["small-business-support", "Small Business Support"],
  ["oem-acquisition-initiatives", "OEM-Acquisition Initiatives"],
]);
const audienceMap = new Map([
  ["contracts-acquisitions", "Contracts/Acquisitions"],
  ["program-operations", "Program/Operations"],
  ["security-compliance", "Security/Compliance"],
  ["industry-all-businesses", "Industry (All Businesses)"],
]);
let clearAllButton,
...
if (document.getElementById("news") != null) {
  ...
  function initPagination() {
    ...
    document.getElementById("articles-count").innerHTML =
  ...
  function initalizeNewsFacets() {
    newsFacets = [
      document.getElementById("news-topic-area"),
      document.getElementById("news-publisher"),
    ];
  ...
  function updateNewsResults() {
    ...
    const selectedFacets = newsFacets.map((facet) =>
      Array.from(facet.querySelectorAll("input:checked")).map(
        (input) => input.value
      )
    );
  ...
  function getFilteredNews(selectedFacets) {
    const filteredArticles = articles.filter((currNew) => {
      const [selectedTopic, selectedPublisher] = selectedFacets;
      const topicAreaMatch =
        selectedTopic.length === 0 || selectedTopic.includes(currNew.topic);
      const publisherMatch =
        selectedPublisher.length === 0 ||
        selectedPublisher.includes(currNew.publisher);
  ...
  function displayArticles(page) {
    ...
    for (let i = startIndex; i < endIndex && i < filteredArticles.length; i++) {
      ...
      if (filteredArticles[i].synopsis != "") {
        articleLink =
          baseUrl +
          filteredArticles[i].path.replace("_news", "").replace(".md", ""); //!!File slug
      }
      ...
      displayArticles += `<div class="article-card-container">
  ...
  function createPaginationButtons(totalPages) {
    ...
    startButton.ariaLabel =
      "This button direct to the first page of the articles";
    ...
    prevButton.ariaLabel =
      "This button will direct to the previous page articles";
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      button.addEventListener("click", () => {
        ...
        if (acActive[0] != null)
          acActive[0].classList.remove("articles-page-active");
        setPagination();
        button.classList.add("articles-page-active");
        displayArticles(currentPage);
        document
          .getElementById("article-heading")
  ...
  function getFromToButton() {
    ...
    return Array.from(
      { length: Math.min(totalPages, pageMax) },
      (_, i) => i + 1 + from
    );
  }
}
...
try {
  ...
  if (document.getElementById("page-announcements") != null) {
    pageHeadingHeight +=
      document.getElementById("page-announcements").offsetHeight;
  }
} catch (error) {
  //If Inner page
  pageHeadingHeight =
    document.getElementById("inner-page-heading").offsetHeight;
}

if (document.getElementById("page-directory") != null) {
  //Other page beside homepage contain page-directory.
  populateDirectory();
  initalizeTabIndex();
  initalizePageNav();
  directToTab();
...
function searchURLParam() {
  ...
  if (filters != null) {
    const filterCheckboxMap = new Map([
      ["policy", "check1"],
      ["acquisition-best-practices", "check2"],
      ["small-business", "check3"],
      ["market-intelligence", "check4"],
      ["technology", "check5"],
      ["contract-solutions", "check6"],
      ["itvmo-general", "check7"],
      ["contracts-acquisitions", "check8"],
      ["program-operations", "check9"],
      ["security-compliance", "check10"],
      ["industry-all-businesses", "check11"],
      ["guidance", "check12"],
      ["use-case", "check13"],
      ["report", "check14"],
      ["information-slick", "check15"],
      ["tool", "check16"],
      ["public-resource", "check17"],
      ["goverment-military-access-only", "check18"],
    ]);
...
function initalizeFacets() {
  ...
  facets = [
    document.getElementById("topic-area"),
    document.getElementById("audience"),
    document.getElementById("resource-type"),
    document.getElementById("resource-source"),
...
function getFilteredRecources(selectedFacets) {
  const filteredResources = resources.filter((resource) => {
    ...
    const totalAreaMatch =
      selectedTotalArea.length === 0 ||
      selectedTotalArea.includes(resource.filter);
    const audienceMatch =
      selectedAudience.length === 0 ||
      selectedAudience.includes(resource.audience);
    const resourceTypeMatch =
      selectedResourceType.length === 0 ||
      selectedResourceType.includes(resource.resourceType);
    const sourceMatch =
      selectedSource.length === 0 || selectedSource.includes(resource.govOnly);
...
function displayResults(filterResources) {
  ...
  if (filterResources.length === 0) {
    ...
    filteredResults.innerHTML =
      `<p id="resources-count">${resourcesCount} Items </p>` + resourceCardList;
  }
}
...
$(function () {
  function updateNavPosition(
    ...
    if (
      scrollTop >= 0 &&
      scrollTop < totalHeightAbove &&
      scrollTop < contentNavHeight - $navList.height()
    ) {
      ...
      $navList
        .removeClass("fixed")
        .addClass("absolute")
        .css("top", contentNavHeight - $navList.height());
    }
  }
...
function displayTabContent(aButton) {
  ...
  setTimeout(function () {
    aButton.scrollIntoView({ behavior: "smooth" });
  }, 500); //Wait until the displayTabContent animation is over before scroll to the section.
}
...
function populateDirectory() {
  ...
  for (let i = urlSplit.length - 1; i >= 0; i--) {
    ...
    if (
      currentUrl ==
      "https://federalist-ce2ad52a-cc88-456b-a4c1-74c6e5887c73.sites.pages.cloud.gov/preview/gsa/itvmo/News-filter/"
    ) {
      newElements += `<a href="${currentUrl}">Home</a>`;
      break;
    }
    if (i == urlSplit.length - 1) {
      //The page that the user currently on, make it not a link.
      newElements += `<p>${cp}</p><img src="${baseUrl}/assets/images/icons/directory-arrow.svg" alt="next arrow">`;
    } else if (i > 0) {
      console.log(oldCp);
      if (
        cp == "News" &&
        oldCp !== "Annual Leading Edge Technologies Report" &&
        oldCp !== "Quarterly Itvmo Newsletter" &&
        oldCp !== "Previous Quarterly Itvmo Newsletter" &&
        oldCp !== "Informatica's Data In Action Summit"
      ) {
...
function urlToString(str) {
  ...
  capitalizedStr = capitalizedStr.replace(
    /-(\w)/g,
...
function retriveEventsData() {
  ...
  const rawPastHighlights = document.getElementsByClassName(
    "raw-past-event-highlight-data"
  );
  ...
  for (ev of rawEventList) {
    const startTime = new Date(
      ev.getAttribute("data-st").replace(" ", "T").replace(" +", "+")
    );
    const endTime = new Date(
      ev.getAttribute("data-et").replace(" ", "T").replace(" +", "+")
    );
...
async function displayEvents(navList, tabId, currEventList) {
  ...
  for (let i = 0; i < currEventList.length; i++) {
    let currEventMonth = `${
      currEventList[i].date.getMonth() + 1
    }${currEventList[i].date.getFullYear()}`;
    if (currMonth == "") {
      ...
      currMonthDiv = setMonthDiv(
        eventContainer,
        currMonth,
        months[currEventList[i].date.getMonth()],
        currEventList[i].date.getFullYear()
      );
    }
    ...
    if (currMonth != currEventMonth + tabId) {
      setPageNav(
        ...
        currMonth,
        monthEventCount,
        months[currEventList[i - 1].date.getMonth()],
        currEventList[i - 1].date.getFullYear(),
        active
      ); //i-1 because the current array already change to the next month.
    ...
    setEventDiv(
      currMonthDiv,
      currEventList[i],
      `${months[currEventList[i].month]} ${
        currEventList[i].dateNumber
      }, ${currEventList[i].date.getFullYear()}`
    );
  }
}
...
async function runCalendar(d, m, y) {
  ...
  if (nav !== 0) dt.setDate(1);
  else if (d !== null) dt.setDate(d);
  ...
  document.getElementById("monthDisplay").innerText = `${monthString} ${year}`;
  ...
  for (let i = 1; i <= paddingDays + daysInMonth; i++) {
    ...
    if (i > paddingDays) {
      ...
      for (var a = 0; a < thisMonthEvents.length; a++) {
        if (
          thisMonthEvents[a].date.getMonth() == monthNum &&
          thisMonthEvents[a].date.getDate() == i - paddingDays &&
          thisMonthEvents[a].date.getFullYear() == year
        ) {
      ...
      daySquare.ariaLabel = `${dt.toLocaleDateString("en-us", {
        month: "long",
      })} ${i - paddingDays} event count of ${eventForDay.length}`;
      ...
      daySquare.onkeydown = function (key) {
...
function populateHighlight() {
  ...
  for (i = 0; i < slides.length; i++) {
    if (highlightArray[parseInt(order[i].textContent) - 1] == null) {
      highlightArray[parseInt(order[i].textContent) - 1] =
        slides[i].cloneNode(true);
    } else {
...
function initHighlightButtons() {
  ...
  prev.onkeydown = function (key) {
    previousSlide(key);
  };
  next.onkeydown = function (key) {
    nextSlide(key);
  };

  function nextSlide(key) {
    if (key.keyCode === 32 || key.keyCode === 13) showSlides();
  }
  function previousSlide(key) {
    if (key.keyCode === 32 || key.keyCode === 13) prevSlide();
  }
}
